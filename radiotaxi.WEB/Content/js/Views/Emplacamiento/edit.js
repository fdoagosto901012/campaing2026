$(document).ready(function () {
    ...
    VeeValidate.Validator.extend('vin', {
        getMessage: field => 'El número VIN no puede contener las letras I, O o Q, y debe tener exactamente 17 caracteres.',
        validate: value => {
            ...
            // Si contiene I, O o Q, es inválido
            if (/[IOQ]/.test(trimmed)) {
                return false;
            }

            // Solo debe tener letras A-Z y dígitos
    ...
    var app = new Vue({
        ...
        methods: {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        $.each(data, function (index, value) {
                            html += "<option value=\"" + value.Id_Modelo + "\">" + value.Modelo + "</option>";
                        });
                        $("#Id_Modelo").html(html);
                    },
                    complete: function (response) {
            ...
            onUser: function (event) {
                ...
                $.ajax({
                    type: "POST",
                    url: "/general/getUserTaxi/",
                    data: { PartnerReference: value },
                    context: this,
                    cache: false,
                    success: function (data) {
                        console.log(data);
                        html = data.firstName + " " + data.lastNameF + " " + data.lastNameM;
